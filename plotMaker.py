...
def getAllPlots(savefolder, basefolder, matname, targetType, headings, l1step):
    ...
    # plot obj ratios
    objs_scaled_df.plot(x=np.arange(objs_scaled.shape[0]), kind='box',
                        title="Objective function ratios to CG Unit for f=" + targetType, meanline=True, showmeans=True,
                        logy=True)
    ...
    # plot runtimes
    runTime_df.plot(x=np.arange(objs_scaled.shape[0]), kind='box',
                    title="Run time in seconds per method for f=" + targetType, meanline=True, showmeans=True)
    plt.xlabel('Method')
    ...
    # plot MU ratios
    mus_scaled_df.plot(x=np.arange(objs_scaled.shape[0]), kind='box', title="MU ratios to CG Unit for f=" + targetType,
    ...
    for k in range(len(kFix)):
        for l in range(len(l1Fix)):
            ...
            for s, i in zip(objs_scaled_df.columns.values, range(len(objs_scaled_df.columns.values))):
                color = rainbow[i]
                ax.plot(mus_scaled_df.xs((kFix[k], l1Fix[l]), level=(0, 1))[s],
                        objs_scaled_df.xs((kFix[k], l1Fix[l]), level=(0, 1))[s], color=color, label=s, marker='o',
                        zorder=2)
            if counter == 1:
            ...
            plt.plot((0, mus_scaled_df.xs((kFix[k], l1Fix[l]), level=(0, 1)).max().max()), (1, 1), color='orange',
                     zorder=1)
            plt.plot((1, 1), (objs_scaled_df.xs((kFix[k], l1Fix[l]), level=(0, 1)).min().min(),
                              objs_scaled_df.xs((kFix[k], l1Fix[l]), level=(0, 1)).max().max()), color='orange',
                     zorder=1)
            plt.axis('tight')
            plt.xlim(min(0.5, mus_scaled_df.xs((kFix[k], l1Fix[l]), level=(0, 1)).min().min()),
                     mus_scaled_df.xs((kFix[k], l1Fix[l]), level=(0, 1)).max().max())
            plt.ylim(min(0.7, objs_scaled_df.xs((kFix[k], l1Fix[l]), level=(0, 1)).min().min()),
                     objs_scaled_df.xs((kFix[k], l1Fix[l]), level=(0, 1)).max().max())
            counter += 1
    fig = plt.gcf()
...
savefolder = '/media/troy/DataDrive/Dropbox/PythonLS/leafseq/figsNew/'
basefolders = ['stepOrder2ClinK/', 'stepOrder0ClinK/', 'doublesinClinK/', 'singlesinClinK/', 'erfsClinK/']
